            ...
            newData = newData.map(item => {
                if (item.id === record.id && state.action !== "Liste des actions") 
                {
                    item.interpretations.push({
                        action: (actionsList.find(action => action.id === state.action) || actionsList[0]),
            ...
            <List
                ...
                renderItem={(item: IInterpretations) => (
                    <List.Item className="list-item">
                        ...
                        <Typography.Text>
                            {item.content}
                            <Tag color="success" className="action">Action {item.action.number_action}</Tag>
                        </Typography.Text>
                    </List.Item>
                )}
            />
            <Input.Group compact >
                <Input.TextArea value={state.inpretation} onChange={handleChange} placeholder="Votre interpretation" style={{ width: '50%' }} />
                <Select value={state.action} onSelect={handleSelect} style={{ width: '50%' }}>
                    {
                        actionsList.map((item: IAction) => <Option key={item.id} value={item.id + ""}>Action {item.number_action}</Option>)
                    }
                </Select>
                <Button style={{ marginTop: "1rem" }} type="primary" onClick={handleSubmit} icon={<PlusOutlined />}>Ajouter</Button>
            </Input.Group>
        </>
    }
    const columns: any[] = [
