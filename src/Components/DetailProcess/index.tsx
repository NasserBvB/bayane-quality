...
import { Breadcrumb, Button, Col, Input, Popover, Row } from "antd";
...
export const DetailProcess = ({ page }: any) => {
  const { currentProcessus, toggleSearching } = useGlobal();
    ...
    <>
      ...
      >
        <Input
          ...
          addonAfter={<SearchOutlined color="blue" />}
        />
      </Row>
      <Row
        style={{ display: "flex", justifyContent: "space-between" }}
      ...
      >
        ...
        >
          <Breadcrumb.Item href="">{currentProcessus.title}</Breadcrumb.Item>
          <Breadcrumb.Item href="">{page}</Breadcrumb.Item>
        </Breadcrumb>
        <Popover
          content={FormsActions}
          trigger="hover"
          placement="topRight"
          arrowPointAtCenter
        >
      ...
      <Row
        style={{ border: "1px solid #6666", marginBottom: "10px" }}
        gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}
      >
        <Col className="gutter-row" span={6}>
        ...
        <Col className="gutter-row" span={8}>
          <div style={style}>
            {currentProcessus.pilote.map((item: string) => (
              <div key={item} color="blue">
                {item}
              </div>
            ))}
          </div>
        ...
        <Col className="gutter-row" span={6}>
          <div style={{ ...style, textTransform: "uppercase" }}>
            {currentProcessus.code}
          </div>
        </Col>
      </Row>
    </>
